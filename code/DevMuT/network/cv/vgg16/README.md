...
```bash
# run training example
python train-VOC.py --config_path=[YAML_CONFIG_PATH] --device_target="GPU" --dataset=[DATASET_TYPE] --data_dir=[DATA_PATH] > output.train.log 2>&1 &
...
# (5) Set the startup file to "train-VOC.py" on the website UI interface.
...
```bash
usage: train-VOC.py [--config_path YAML_CONFIG_PATH]
...
```bash
python train-VOC.py --config_path=/dir_to_code/cifar10_config.yaml --data_dir=your_data_path > out.train.log 2>&1 &
```
...
```bash
python train-VOC.py  --config_path=/dir_to_code/imagenet2012_config.yaml --device_target="GPU" --dataset="imagenet2012" --is_distributed=0 --data_dir=$DATA_PATH  > output.train.log 2>&1 &
```
