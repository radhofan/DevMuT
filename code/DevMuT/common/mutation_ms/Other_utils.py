...
def scan_same_inout(model, layer1_name, scan_layers):
    candidate_layers = []
    for layer_name in scan_layers:
        if layer_name == layer1_name:
            continue
        layer1_inshape = model.in_shapes[layer1_name]
        layer1_outshape = model.out_shapes[layer1_name]
        layer2_inshape = model.in_shapes[layer_name]
        layer2_outshape = model.out_shapes[layer_name]
        if layer1_inshape == layer2_inshape and layer1_outshape == layer2_outshape:
            candidate_layers.append(layer_name)

    if len(candidate_layers) == 0:
        return False
    else:
        return np.random.permutation(candidate_layers)[0]
...
def judge_legenacy(model, input_size, mutate_logger=None, train_configs=None):
    ...
    for i in range(len(input_sizes)):
        ...
        test_data_numpy = np.ones(input_size_tuple).astype(np_dtype)
        test_input = mindspore.Tensor(test_data_numpy, dtype)
    ...
    try:
        ...
        if mutate_logger is not None:
            mutate_logger.info("Exception information: " + str(e))
        return False
...
def get_alternative_Basicops(in_shape, out_shape, mut_type):
    ...
    if not dimension == "2D":
        ...
        del alternative_insert_layers['dense']
        # have not add layernorm

    alternative_insert_layers_instances = get_new_basicop(alternative_insert_layers, activation_names,
                                                          insert_inchannels, insert_outchannels, kernel_size, stride,
                                                          dimension)

    return alternative_insert_layers_instances


def get_alternative_Cascadeops(in_shape, out_shape, mut_type):
    if mut_type == "CM" or mut_type == "RA":
        insert_inchannels = in_shape[1]
    elif mut_type == "LA" or mut_type == "LC":
        insert_inchannels = out_shape[1]
    ...
    activations_alter = BasicOPUtils.available_activations()
    acts = np.random.permutation(list(activations_alter.keys()))
    alternative_insert_layers = get_new_cascadeop(alternative_insert_layers, insert_inchannels, insert_outchannels,
                                                  kernel_size, stride=1, activation=acts[0])
    return alternative_insert_layers


def ms_deepcopy(old_model):
    stree = SymbolTree.create(old_model)
    copy_model = stree.get_network()
    return copy_model


def get_alternative_Basicops_followlog(in_shape, out_shape, mut_type):
    if mut_type == "CM" or mut_type == "RA":
        insert_layer_inshape = deepcopy(in_shape)
        insert_inchannels = in_shape[1]
    elif mut_type == "LA" or mut_type == "LC":
...
def get_alternative_Cascadeops_followlog(in_shape, out_shape, mut_type, activation_name):
    if mut_type == "CM" or mut_type == "RA":
        insert_inchannels = in_shape[1]
    elif mut_type == "LA" or mut_type == "LC":
        insert_inchannels = out_shape[1]
    ...
    alternative_insert_layers = CascadeOPUtils().extension_ops
    alternative_insert_layers = get_new_cascadeop(alternative_insert_layers, insert_inchannels, insert_outchannels,
                                                  kernel_size, stride=1, activation=activation_name.lower())
    return alternative_insert_layers


def set_layer(model, layer, mutlayer_name, f, mut_type, generations, mutate_logger):
    try:
        model.set_layers(mutlayer_name, layer)
        return True
    except Exception as e:
        f.write("Failed to set layer: {}!\n".format(mutlayer_name))
        f.write(str(e) + "\n")
        f.write("mut_result:{}\n".format("{} set layers failure!".format(mut_type)))
        f.write("{} generation!\n\n".format(generations))

        mutate_logger.info("Failed to set layer: {}!".format(mutlayer_name))
        mutate_logger.info(str(e))
        mutate_logger.info("mut_result:{}".format("{} set layers failure!".format(mut_type)))
        mutate_logger.info("{} generation!\n".format(generations))
        return "{} set layers failure!".format(mut_type)


def set_layer_nolog(model, layer, mutlayer_name, mut_type):
    try:
        model.set_layers(mutlayer_name, layer)
        return True
    except Exception as e:
...
def create_replacecell(input_shape, output_shape):
    replace_cell = Replace_ms(input_shape, output_shape)
    return replace_cell
...
def get_lubrication_op(inshape, layer, input_size):
    ...
    if len(newshape) == 0:
        ...
        return Replace_ms(inshape, newshape)
